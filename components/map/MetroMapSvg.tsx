...
// převzatá mapa/PNG/PDF DPP.
export default function MetroMapSvg({ currentViewBox, selectedStationId, onSelectStation }: MetroMapSvgProps) {
  return (
    <svg viewBox={currentViewBox} className="h-full w-full touch-none select-none" role="img" aria-label="Schematická mapa pražského metra, linky A, B a C">
        ...
        return (
          <g key={node.id}>
            <circle
              ...
              tabIndex={0}
              role="button"
              aria-label={`Stanice ${node.name}, linka ${node.lines.join(", ")}`}
