...
export default function DemoLocationPicker({ positions, onSelect }: DemoLocationPickerProps) {
  if (positions.length === 0) return null;

  return (
    <div className="mt-3 rounded-xl border border-dashed border-gray-300 p-3">
      <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Demo poloha (jen dev)</p>
      <div className="mt-2 flex flex-wrap gap-2">
        {positions.map((position) => (
          <button
            ...
            className="min-h-[36px] rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-700 hover:border-gray-900"
          >
            {position.label}
          </button>
        ))}
